...
MyString& MyString::assign(const MyString& o) {
  ...
  for (int i = 0; i != o.str_len; i++) str[i]=o.str[i];
  str_len = o.str_len + 1;
  return *this;
};
...
void MyString::reserve(int size) {
  if(size > memory_cap) {
    ...
    str = new char[size];
    memory_cap = size;
    for(int i = 0; i != str_len; i++) str[i] = prev_string_content[i];
    delete [] prev_string_content;
  }
};
char MyString::at(int i) const {
  if(i < 0 || i >= str_len) return '\0';
  else return str[i]; 
};
MyString& MyString::insert(int loc, const MyString& o) {
  ...
  if(new_str_len > memory_cap) {
    if(memory_cap * 2 > new_str_len) memory_cap *=2;
    else memory_cap = new_str_len;
    ...
    for(; i < str_len; i++) str[o.str_len + i] = prev_str_content[i];
...
MyString& MyString::erase(int loc, int num){
  if(num < 0 || loc < 0 || loc > str_len) return *this;
  for(int i = loc + num; i < str_len; i++) str[i - num] = str[i];
  str_len -= num;
  return *this;
};
...
int MyString::find(int find_from, MyString &o) const {
  int i, j;
  if(o.str_len == 0) return -1;
  for(i = find_from; i <= str_len - o.str_len; i++) {
    for(j = 0; j < o.str_len; j++){
      if(str[i+j] != o.str[j]) break;
    }
    if(j == o.str_len) return i;
  }
  return -1;
};
...
int MyString::compare(const MyString &o) const {
  for(int i = 0; i < std::min(str_len, o.str_len); i++) {
    if(str[i] > o.str[i]) return 1;
    else if(str[i] < o.str[i]) return -1;
  }
  if(str_len == o.str_len) return 0;
  else if (str_len > o.str_len) return 1;
  return -1;
};
bool MyString::operator==(MyString &o) {
  if(this->compare(o) == 0) return true;
  return false;  
};
void testexplicit(MyString s) {
  s.println();
};
...
int main() {
  ...
  std::cout << "find second very : " << str1.find(str1.find(0, "very") + 1 , "very") << std::endl;
  ...
  if (str2 == str2) std::cout << "compare str2 with str2 with operator overloading : true" << std::endl;
  // [] overloading
